...
export function fromOrderRow(row: OrderRow): Order {
  return {
    ...
    previsaoInicio: row.previsao_inicio ?? null,
...
export function toOrderRow(row: Order) {
  return {
    ...
    filamento_ids: row.filamentoIds?.length ? row.filamentoIds : null,
    previsao_inicio: row.previsaoInicio ?? null,
    previsao_entrega: row.previsaoEntrega ?? null,
