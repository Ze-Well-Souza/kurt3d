...
export async function orderPartsRepo() {
  ...
  const rows = unwrapResult(
    await supabase.from("order_parts").select("*").order("order_id").order("position"),
    {
      table: "order_parts",
      operation: "list",
  ...
  return {
    ...
    async saveForOrder(orderId: string, next: ReturnType<typeof fromOrderPartRow>[]) {
      const current = unwrapResult(
        await supabase.from("order_parts").select("id").eq("order_id", orderId),
        {
          table: "order_parts",
      ...
      if (next.length > 0) {
        unwrapResult(
          await supabase
            .from("order_parts")
            .upsert(next.map(toOrderPartRow) as never[], { onConflict: "id" }),
          {
            table: "order_parts",
