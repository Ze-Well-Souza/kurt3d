...
export async function filamentoInstallmentsRepo() {
  ...
  const rows = unwrapResult(
    await supabase
      .from("filamento_payment_installments")
      .select("*")
  ...
  return {
    ...
    async insertMany(items: ReturnType<typeof fromInstallmentRow>[]) {
      ...
      unwrapResult(
        await supabase.from("filamento_payment_installments").insert(items.map(toInstallmentRow)),
        {
          table: "filamento_payment_installments",
    ...
    async update(installment: ReturnType<typeof fromInstallmentRow>) {
      unwrapResult(
        await supabase
          .from("filamento_payment_installments")
          .update(toInstallmentRow(installment))
    ...
    async deleteByPayment(paymentId: string) {
      unwrapResult(
        await supabase.from("filamento_payment_installments").delete().eq("payment_id", paymentId),
        {
          table: "filamento_payment_installments",
