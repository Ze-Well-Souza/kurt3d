...
export async function insumoInstallmentsRepo() {
  ...
  const rows = unwrapResult(
    await supabase
      .from("insumo_payment_installments")
      .select("*")
  ...
  return {
    ...
    async insertMany(items: ReturnType<typeof fromInsumoInstallmentRow>[]) {
      ...
      unwrapResult(
        await supabase
          .from("insumo_payment_installments")
          .insert(items.map(toInsumoInstallmentRow)),
        {
          table: "insumo_payment_installments",
    ...
    async update(installment: ReturnType<typeof fromInsumoInstallmentRow>) {
      unwrapResult(
        await supabase
          .from("insumo_payment_installments")
          .update(toInsumoInstallmentRow(installment))
    ...
    async deleteByPayment(paymentId: string) {
      unwrapResult(
        await supabase.from("insumo_payment_installments").delete().eq("payment_id", paymentId),
        {
          table: "insumo_payment_installments",
