...
export async function filamentoPaymentsRepo() {
  ...
  const rows = unwrapResult(
    await supabase.from("filamento_payments").select("*").order("created_at", { ascending: false }),
    {
  ...
  return {
    ...
    async update(payment: ReturnType<typeof fromPaymentRow>) {
      unwrapResult(
        await supabase
          .from("filamento_payments")
          .update(toPaymentRow(payment))
          .eq("id", payment.id),
        {
          table: "filamento_payments",
          operation: "update",
    ...
    async attachToBatch(batchId: string, paymentId: string) {
      unwrapResult(
        await supabase
          .from("filamentos")
          .update({ payment_id: paymentId, batch_id: batchId })
          .eq("batch_id", batchId),
        {
          table: "filamentos",
    ...
    async detachFromFilamentos(paymentId: string) {
      unwrapResult(
        await supabase.from("filamentos").update({ payment_id: null }).eq("payment_id", paymentId),
        {
          table: "filamentos",
