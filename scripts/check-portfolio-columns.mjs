...
async function main() {
  ...
  const probeCols = [
    ...
    "custo_kwh",
    "consumo_kw",
    "custo_mao_obra_horas",
    "custo_mao_obra_valor_hora",
    "taxa_gateway",
    "perda_percent",
  ];
  for (const col of probeCols) {
    const r = await fetch(`${URL_BASE}/rest/v1/portfolio_projects?select=${col}&limit=1`, {
      headers,
    });
    console.log(
      `column ${col}:`,
      r.status === 200 ? "OK" : `MISSING (${r.status}) ${JSON.stringify(await r.json())}`,
    );
  }
